...
from modulos.pokeday import verificar_calendario_pokemon , encontrar_cor_tipo
...
def create_evolution_grid(evolution_chain):
    ...
    if len(variants) == 2:        
        ...
        for i, evo in enumerate(evolution_chain):
            ...
            draw.text((x_offset + 40, 180), evo['name'], font=ImageFont.truetype("font/KoomikMono-Regular.otf",16), fill="black")
            ...
            if i < len(evolution_chain):
                ...
                if next_evo['method'] is not None:
                    ...
                    # Desenhando o texto
                    draw.text((x_offset - 40, 110), f"{method_text}\n{level_text}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black", align="center")
        ...
        for i, evo in enumerate(evolution_chain):
            ...
            draw.text((root_offset[0] + x_offset + 10, root_offset[1] + y_offset + 70), evo['name'], font=ImageFont.truetype("font/KoomikMono-Regular.otf",14), fill="black")
       ...
       #     print(r)
        ...
        for i, pokemon in enumerate(evolution_chain):
            ...
            if base_img:
                ...
                draw.text((x_base + 10, y_base_offset + 110), base_name, font=ImageFont.truetype("font/KoomikMono-Regular.otf", 14), fill="black")
            ...
            if num_variations > 0:
                ...
                for j, variation in enumerate(variations):
                    ...
                    if variation_img:
                        ...
                        draw.text((x_variation + 10, y_variation + 90), variation_name, font=ImageFont.truetype("font/KoomikMono-Regular.otf", 12), fill="black")
                    ...
                    draw.text((x_variation + 10, y_variation + 110), f"{method_text}\n{level_text}", font=ImageFont.truetype("font/KoomikMono-Regular.otf", 10), fill="black")
        """
    #return img
"""
...
async def montarpokedex(self,interaction: discord.Interaction,pokemon , brilhante, pagina , originaluser):
    ...
    specie = requests.get(f"https://pokeapi.co/api/v2/pokemon-species/{dex['species']['name']}/").json()
    aniversario = await verificar_calendario_pokemon(pokemon_nome=dex['species']['name'])
    try:
        if brilhante is True:
            imagempokemon = Image.open(requests.get(dex['sprites']['other']['home']['front_shiny'], stream=True).raw)
        else: imagempokemon = Image.open(requests.get(dex['sprites']['other']['home']['front_default'], stream=True).raw)
    except:
        imagempokemon = Image.open("imagens/Pokedex/Interrogation.png")

    #MONTAGEM INICIAL
    corfundo = encontrar_cor_tipo(dex['types'][0]['type']['name'])
    nomepokemon = dex['species']['name'].capitalize() #DEFININDO NOME
    idpokemon = f"#{dex['id']}" #DEFININDO ID
    fundo = Image.new("RGBA",(800,300), corfundo) #DEFININDO FUNDO
    pokeboladefundo = Image.open("imagens/Pokedex/pokeballbranca.png").resize((200,200))
    ...
    if brilhante is True:
        shiny = Image.open("imagens/Pokedex/Shiny.png")
    ...
    fundodraw.text((20,4), nomepokemon ,font=ImageFont.truetype("font/PoetsenOne-Regular.ttf",30))
    fundodraw.text((20,37), idpokemon ,font=ImageFont.truetype("font/KoomikMono-Regular.otf",18))

    # INDICAR ANIVERSARIO DO POKÈMON
    if aniversario is not None:
        encaixebolo = fundodraw.textbbox((0,0), idpokemon, font=ImageFont.truetype("font/KoomikMono-Regular.otf", 18))
        bolo = Image.open("imagens/Pokedex/Cake.png")
        fundo.paste( bolo.resize((22,22)) , (30 + encaixebolo[2],40) ,bolo.resize((22,22)) ) #COLANDO POKÈMON
        fundodraw.text((55 + encaixebolo[2],37), f"{aniversario.day:02d}/{aniversario.month:02d}" ,font=ImageFont.truetype("font/KoomikMono-Regular.otf",18))
    ...
    text_widths = [drawfundobranco.textbbox((0, 0), word, font=ImageFont.truetype("font/KoomikMono-Regular.otf",18))[2] for word in texto]
    space_width = (total_width - sum(text_widths)) // (len(texto) - 1)
    x = 300
    for i, word in enumerate(texto):
        fundodraw.text((x, 10), word, font=ImageFont.truetype("font/KoomikMono-Regular.otf",20), fill="black")
    ...
    if pagina == 0:
        ...
        #Titulos
        fundodraw.text((340, 65), Res.trad(interaction=interaction,str="Dados"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",18), fill="black",align="center")
        fundodraw.text((590, 65), Res.trad(interaction=interaction,str="Reprodução"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",18), fill="black",align="center")
        fundodraw.text((610, 160), Res.trad(interaction=interaction,str="Genero"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",18), fill="black",align="center")

        #DEMAIS DADOS
        fundodraw.text((270, 100), Res.trad(interaction=interaction,str="Especie"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 100), caracteristica, font=ImageFont.truetype("font/KoomikMono-Regular.otf",10 if len(caracteristica) > 19 else 12), fill="black")
        fundodraw.text((270, 130), Res.trad(interaction=interaction,str="Altura"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 130), f"{dex['height']/10} M", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black")
        fundodraw.text((270, 160), Res.trad(interaction=interaction,str="Peso"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 160), f"{dex['weight']/10} Kg", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black")
        fundodraw.text((270, 190), Res.trad(interaction=interaction,str="Tipagem"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 190), tipo[:22], font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black")
        fundodraw.text((270, 220), Res.trad(interaction=interaction,str="Habilidades"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 220), habilidades, font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black", spacing=1)
        fundodraw.text((540, 100), Res.trad(interaction=interaction,str="Grupo_de_Ovos"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((660, 100), egggroup, font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black")
        fundodraw.text((540, 130), Res.trad(interaction=interaction,str="eclodir"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((660, 130), hattime, font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        fundodraw.text((540, 225), Res.trad(interaction=interaction,str="Taxa_de_Captura"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((660, 225), f"{specie['capture_rate']}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        
        # MONTAGEM GENERO E OUTRAS COISAS
        if specie['gender_rate'] <= 0 : 
            fundodraw.text((540, 200), Res.trad(interaction=interaction,str="Sem_Dados_genero"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959",align="center")
        else:
            ...
            Male = Image.open("imagens/Pokedex/Male.png").resize((15,15)) #ICONE MACHO
            fundo.paste ( Male , (560,200) , Male)
            fundodraw.text((580, 200), f"{macho:.2f}%", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
            Female = Image.open("imagens/Pokedex/Female.png").resize((15,15)) #ICONE FEMEA
            fundo.paste ( Female , (670,200) , Female)
            fundodraw.text((690, 200), f"{femea:.2f}%", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        ...
        fundodraw.text((280, 45), description_wrapped, font=ImageFont.truetype("font/KoomikMono-Regular.otf",14), fill="black")
        fundodraw.text((280, 140), "HP", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 140), f"{hp}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
                                    #  X1  Y1   X2  Y2
        ...
        fundodraw.text((280, 160), Res.trad(interaction=interaction,str="Ataque"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 160), f"{attack}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        ...
        fundodraw.text((280, 180), Res.trad(interaction=interaction,str="Defesa"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 180), f"{defense}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        ...
        fundodraw.text((280, 200), "SP.ATK", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 200), f"{special_attack}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        ...
        fundodraw.text((280, 220), "SP.DEF", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 220), f"{special_defense}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        ...
        fundodraw.text((280, 240), Res.trad(interaction=interaction,str="Velocidade"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959")
        fundodraw.text((370, 240), f"{speed}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
        ...
        fundodraw.text((280, 260), Res.trad(interaction=interaction,str="Total"), font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="#595959",align="center")
        fundodraw.text((370, 260), f"{hp + attack + defense + special_attack + special_defense + speed}", font=ImageFont.truetype("font/KoomikMono-Regular.otf",12), fill="black",align="center")
    else:
...
class PokemonCache:
    ...
    async def load_pokemon_data(self):
        if not self.loaded:
            ...
            self.pokemon_data = [
                {"id": pokemon['url'].split('/')[-2], "name": pokemon['name']}
...
class pokedex(commands.Cog):
  def __init__(self, client: commands.Bot):
    self.client = client
    self.pokemon_cache = PokemonCache()  # Instância do cache de Pokémon
  ...
  async def on_ready(self):
    ...
    await self.pokemon_cache.load_pokemon_data()
...
# sistema de autocomplete da lista de pokemon
  async def pokemon_autocomplete(self,interaction: discord.Interaction, current: str):
    # Aguarda a função carregar os dados na memória se ainda não estiverem carregados
    await self.pokemon_cache.load_pokemon_data()

    # Filtra os nomes com base no que o usuário digitou
    filtered_pokemons = [
        pokemon for pokemon in self.pokemon_cache.get_pokemon_data()
        if current.lower() in pokemon['name'].lower()
    ]
    # Retorna uma lista de escolhas com o ID e nome do Pokémon
    return [ app_commands.Choice(name=f"{pokemon['id']} - {pokemon['name']}", value=pokemon['name'])  for pokemon in filtered_pokemons ][:25]  # Limite de 25 sugestões
